<template>
  ...
  >
    ...
    <div
      ref="scrollContainer"
      class="scroll-bar flex flex-col flex-1 px-1 mx-0.5 overflow-y-auto min-h-0"
    >
      <ChatBubble
...
const storeRoom = computed(
  () => messagesStore.byRoom[roomId.value] || ({ items: [] } as any)
);
...
async function handleSend() {
  try {
    ...
    if (editingMessageId.value != null) {
      await messagesStore.messageEdit(
        rid,
        Number(editingMessageId.value),
        content
      );
...
function scrollToBottom() {
  ...
  if (!el) return;
  // Use rAF twice to ensure layout is flushed after new VDOM patch
...
// Auto-scroll when message list changes (new message appended or history loaded)
watch(
  () => (storeRoom.value.items || []).length,
  async () => {
    await nextTick();
    scrollToBottom();
  }
...
onBeforeUnmount(() => {
  ...
  try {
    messagesStore.setActiveRoom(null);
  } catch {} // To-do : need to fix that there should be more that one room possibly active, I think that why we have problems with some socket event in the rooms
});
</script>
